    ...
    with open(filename) as f:
        for i, line in enumerate(f):
            ...
            if i == id : 
                ...
                missionName = str(linearray[1])

                vehicle.parameters['INJ_SENSORS'] = int(linearray[2])
                vehicle.parameters['INJ_METHOD'] = int(linearray[3])
                vehicle.parameters['INJ_DELAY'] = int(linearray[4])
                vehicle.parameters['INJ_DURATION'] = int (linearray[5])
                vehicle.parameters['INJ_WP_TRIG'] = int(linearray[6])
                wp_trigger = int(linearray[6])
                vehicle.parameters['INJ_FIELD_X'] = float(linearray[7])
                vehicle.parameters['INJ_FIELD_Y'] = float(linearray[8])
                vehicle.parameters['INJ_FIELD_Z'] = float(linearray[9])
                vehicle.parameters['INJ_MIN'] = int(linearray[10])
                vehicle.parameters['INJ_MAX'] = int(linearray[11])
                vehicle.parameters['INJ_NOISE_D'] = float(linearray[12])
                vehicle.parameters['INJ_NOISE_M'] = float(linearray[13])
...
missionName, wp_trigger = setFaultInjectorParams(int(argv[1]),'faults.csv')

# Upload a mission
upload_mission(missionName)
...
has_header = False
fileExist = os.path.isfile("simulations_report.csv")

with open('simulations_report.csv', 'a+') as outcsv:
    writer = csv.DictWriter(outcsv, fieldnames = ["ID","MISSION", "SENSOR", "METHOD", "DELAY", "DURATION","WP_TRIGGER","X","Y", "Z", "MIN", "MAX", "NOISE_D", "NOISE_M"])
    ...
    writer.writerow( {
        'ID': int(argv[1]),'MISSION' : missionName,'SENSOR': vehicle.parameters['INJ_SENSORS'],
